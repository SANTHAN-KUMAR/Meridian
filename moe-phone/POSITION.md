...
(`G-ROOF v2`); `--dram-gbps` remains assumed (stub S1), and **energy is not measurable at all on
this device without root** (stub S6). Every number of ours lives in an artifact under
...
| **A5** | the NPU should run decode's hot neurons | decode is memory-bound; CPU/GPU may match the NPU there ([HeteroLLM](https://arxiv.org/abs/2501.14794); [2607.05475](https://arxiv.org/abs/2607.05475)) — placement is measured, not assumed. **Constrained 2026-09-14 (see S7):** on the 15R every `/dev/fastrpc-*` node is unopenable by an app *and* by `adb shell`, while `/dev/kgsl-3d0` opens read-write and `libOpenCL.so` is in the app-dlopen allowlist — so in the unprivileged regime the placement question is CPU vs Adreno GPU. Note PowerInfer-2 uses the NPU in **decode**, not only prefill, so this is a real asymmetry in the RQ1 head-to-head and is reported as a limitation | G6  **EXISTENCE PROOF 2026-09-14:** Google AICore ran **Gemini Nano — an actual LLM — on HTP v81 on this unrooted retail phone**, via unsigned PD on CDSP, loading `libQnnHtpV81Skel.so` **and** `libdspqueue_rpc_skel.so` (Qualcomm's *asynchronous* DSP queue, with QoS latency classes). The async queue matters: an earlier objection in this file's discussion — that QNN's synchronous per-dispatch FastRPC cost would sink MoE's ~128 expert dispatches per token — assumed the wrong interface. Not yet a measurement: no tok/s was captured. Evidence: `results/2026-09-14/npu_access_mechanism.log`. |
