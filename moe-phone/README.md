...
| **G6-baseline** | unmodified llama.cpp decode on the 15R: tok/s and flash bytes/token | **IN PROGRESS.** Tooling committed; the first unattended campaign ran while the phone's **lock screen** was up (Termux not foreground), so it is kept as `results/2026-09-16/decode_15r_screenlocked_contaminated/` and **no number from it is reported**. A laptop-side `phone_state_log.csv` now records wakefulness and focus every 30 s so a run can be checked against it | `device/llm_decode_probe.sh`, `device/phone_campaign.sh` → `gates/decode_analyze.py` |
