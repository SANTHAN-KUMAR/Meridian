...
| G2 | routing traces; LRU and Belady hit rate vs cache size; training-free lookahead recall | **RUN 2026-09-14, OLMoE-1B-7B only** (Kaggle 2x T4) → `results/2026-09-14/cache_OLMoE-1B-7B-0924.json`, `traces_OLMoE-1B-7B-0924.npz`. Locality is abundant (Belady far above the no-locality floor) but **LRU captures none of it at phone-sized caches**; training-free lookahead recall is high. The cache result reproduces exactly from the committed traces | `gates/traces_sparsity.py` → `gates/cache_sim.py` |
...
| **G-VALID** | does the roofline reproduce an INDEPENDENT published measurement? | **RUN 2026-09-14** → `results/2026-09-14/byte_budget_validate_powerinfer2.json`. Fed PowerInfer-2's own configuration (TurboSparse-Mixtral, 7 GB, `predicted` mode, d=0.03 for its ~97% exact-zero activations) the roofline lands **within 12%** of their published Fig. 10 rate. It sits 12% *below*, which for an upper bound is a flag, not a pass: three inputs are pessimistic for *their* device (our 15R scattered bandwidth, their un-modelled read bundling, and the S2-inflated resident count). First check of this model against anything outside the repo | `gates/byte_budget.py --densities` |
