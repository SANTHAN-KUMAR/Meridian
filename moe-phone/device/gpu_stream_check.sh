#!/system/bin/sh
# gpu_stream_check.sh — the MoE expert cache ON THE ADRENO GPU while experts stream from flash
...
. $H/thermal_gate.sh
B=$H/ocl-stream
O=$H/gpu_stream_$(date +%Y%m%d_%H%M); mkdir -p "$O"
...
ppl cpu        "-ngl 0 --cpu-strict 1 -C 0xf0"
