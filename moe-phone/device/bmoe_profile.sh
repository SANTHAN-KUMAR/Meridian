...
# 4 compute cores for 30 s of steady decode (software cpu-clock events: the 15R exposes no hardware PMU to shell); `report` by symbol and by thread. Plain (no arena) and
...
for cell in plain arena; do
  ...
  simpleperf stat -p $pid --duration 10 -e cpu-clock,task-clock,page-faults,minor-faults,major-faults,context-switches,cpu-migrations > "$O/$cell.stat.txt" 2>&1
  simpleperf record -p $pid --duration 30 -g -e cpu-clock -f 1000 -o "$O/$cell.perf.data" > "$O/$cell.record.txt" 2>&1
  wait
