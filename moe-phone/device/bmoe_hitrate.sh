...
# about 6 GB. After a reboot and a quiesce the phone reports ~7.6 GB available -- of which the dense
# weights and compute buffers take a fixed share first -- and the simulator says
...
BASE="--chatml -n 32 --ubatch 512 --moe-stream --cache-mb auto --cache-floor-mb 1024 --overlap --dense-weights anon -t 4 --cpu-mask f0 --io-threads 4 --io-cpu-mask 0f"
run() {
  tag=$1_rep$3
...
# NO FORCED BUDGETS. The first version of this script asked for --cache-mb 7000 / 8500 with
# --force-cache, on the reasoning that the phone reports ~7.6 GB free. That reasoning was wrong and it is
# the same mistake that hung the phone at midday: the process needs the cache PLUS the dense weights
# (~1 GB) PLUS its compute buffers (a 1219 MiB CPU compute buffer on the host run), so a forced 7000 MiB
# cache overcommits a 7.6 GB-free phone and pushes it into swap. Here the engine's own auto logic
# decides: --cache-mb auto measures MemAvailable AFTER the dense weights and buffers exist, leaves the
# 1024 MiB floor, and grants min(ceiling, what is actually free). The ceiling is the only thing varied,
