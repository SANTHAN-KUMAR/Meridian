#!/system/bin/sh
# bmoe_pin2.sh — ROTATED-ORDER confirmation of bmoe_pin.sh: compute/IO thread placement for streamed Qwen3-30B-A3B, on a NON-throttled SoC.
#
...
# bmoe_pin.sh (2026-09-17 12:55) ran its cells in a fixed order, so pin_t4 + recycle always ran last in a
# repeat. Here the cell order rotates by one position per repeat (a cyclic Latin square over 5 cells).
#   sh bmoe_pin2.sh REPS
set -u
REPS=${1:-2}
H=/data/local/tmp/moe-stream
. $H/thermal_gate.sh
M=$H/Qwen3-30B-A3B-Q4_0.gguf
O=$H/bmoe_pin2_$(date +%Y%m%d_%H%M); mkdir -p "$O"
...
CELLS="unpinned_t4_io4 pin_t4c47_io4c03 unpinned_recycle pin_t4_recycle pin_t6c27_io2c01_recycle"
flags() {
  case $1 in
    unpinned_t4_io4)          echo "-t 4 --io-threads 4" ;;
    pin_t4c47_io4c03)         echo "-t 4 --cpu-mask f0 --io-threads 4 --io-cpu-mask 0f" ;;
    unpinned_recycle)         echo "-t 4 --io-threads 4 --recycle-pages" ;;
    pin_t4_recycle)           echo "-t 4 --cpu-mask f0 --io-threads 4 --io-cpu-mask 0f --recycle-pages" ;;
    pin_t6c27_io2c01_recycle) echo "-t 6 --cpu-mask fc --io-threads 2 --io-cpu-mask 03 --recycle-pages" ;;
  esac
}
for r in $(seq 1 "$REPS"); do
  set -- $CELLS
  shift_by=$(( (r - 1) % 5 ))
  i=0; while [ $i -lt $shift_by ]; do first=$1; shift; set -- "$@" "$first"; i=$((i + 1)); done
