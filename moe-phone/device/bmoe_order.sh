#!/system/bin/sh
# bmoe_order.sh — does computing a layer's RESIDENT experts first hide the reads of the ones still in
# flight? (patch 0010, --expert-resident-first)
#
...
#
# Cells differ ONLY in the flag; both use the same binary (bmoe-i8mm-order), so the comparison cannot
...
O=$H/bmoe_order_$(date +%Y%m%d_%H%M); mkdir -p "$O"
...
run() {
  ...
  ( cd $H/bmoe-i8mm-order && LD_LIBRARY_PATH=. ./bmoe-cli -m $M $BASE $2 --csv "$O/$tag.csv" -p "$P" > "$O/$tag.out" 2> "$O/$tag.err" )
  echo "exit=$? AFTER $(thermal_state) $(grep -hE 'generation:|moe-stream:|moe-cache:|expert order' "$O/$tag.out" "$O/$tag.err" | tr '\n' ' ')" | tee -a "$O/log.txt"
}
