#!/system/bin/sh
# gpu_stream_check3.sh — rerun with serialized partial uploads (thread-safety fix), after the OpenCL router-reorder fixes (streamed ids: reorder trigger, expert count, row stride). the MoE expert cache ON THE ADRENO GPU while experts stream from flash
...
. $H/thermal_gate.sh
B=$H/ocl-stream3
O=$H/gpu_stream3_$(date +%Y%m%d_%H%M); mkdir -p "$O"
...
# gpu_full already measured three times (11.5482)
