#!/bin/bash
# chain_cap8.sh — cap-8 follow-up (pre-registered in bmoe_gtier.sh 03:05) after chain_tier; derived from chain_tier.sh — tier smoke + A/B only (after the 02:23 smoke's flag bug); derived from chain_night3, which takes over from chain_night2 during the kgsl A/B (which runs on the phone under setsid, independent
# of this host script):
...
log "[cap8] waiting for the cap-3 tier chain (CHAIN_TIER_DONE)"
while [ ! -f "$R/CHAIN_TIER_DONE" ]; do sleep 60; done
reconnect
[ -n "$(lock)" ] && { log "[cap8] phone busy: $(lock)"; exit 1; }
orph=$(A 'ps -A -o ARGS' | grep -E 'bmoe-cli|gx_|sh bmoe_|pinprobe' | grep -v grep)
[ -n "$orph" ] && { log "[cap8] FATAL foreign: $orph"; exit 1; }
...
[ "$want" = "$got" ] || { log "[cap8] FATAL 0023 md5 $got != $want"; exit 1; }
A "chmod 755 $H/bmoe-i8mm-0023/bmoe-cli" >/dev/null
adb push "$MP/device/bmoe_gtier.sh" "$H/" >/dev/null 2>&1 </dev/null
A "grep -q 'GT_CAP=\${GT_CAP:-3}' $H/bmoe_gtier.sh" || { log "[cap8] FATAL pushed script lacks GT_CAP"; exit 1; }
export GTENV="GT_BIN=bmoe-i8mm-0023 GT_VARIANT=1 GT_SPIN=1 GT_CAP=8"
...
log "[cap8] smoke ok=$ok dev=$dev risk=$rk :: $(grep -h 'generation:' "$S"/*.out | tr '\n' ' ') :: $(grep -ho 'host ms:.*' "$S/stack_reps1.err")"
b=$(A "dumpsys battery | grep -m1 ' level' | tr -dc 0-9")
if [ $ok = 1 ] && [ "${b:-0}" -ge 45 ]; then log "[cap8] A/B start (battery $b%)"; campaign bmoe_gtier.sh ab; log "[cap8] A/B pulled: $LAST"; else log "[cap8] A/B not started (smoke ok=$ok, battery ${b}%)"; fi
restore
touch "$R/CHAIN_CAP8_DONE"
