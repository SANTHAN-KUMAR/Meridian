...
controls=(
  ...
  "soa_unfused_fma|s.replace('static void row_block_v(float * acc, int p, uchar16 q, float sc, int off, local const char * yq) {\n    const char16 yl = vload16(0, yq), yh = vload16(0, yq + 16);\n    const int16 lo = convert_int16(q & (uchar16)(15)) - off, hi = convert_int16(q >> (uchar16)(4)) - off;\n    const int16 a = convert_int16(yl), c = convert_int16(yh);\n    const int16 pr = lo * a + hi * c;\n    acc[p * 4 + 0] = fma((float) (pr.s0 + pr.s1 + pr.s2 + pr.s3), sc, acc[p * 4 + 0]);', 'static void row_block_v(float * acc, int p, uchar16 q, float sc, int off, local const char * yq) {\n    const char16 yl = vload16(0, yq), yh = vload16(0, yq + 16);\n    const int16 lo = convert_int16(q & (uchar16)(15)) - off, hi = convert_int16(q >> (uchar16)(4)) - off;\n    const int16 a = convert_int16(yl), c = convert_int16(yh);\n    const int16 pr = lo * a + hi * c;\n    acc[p * 4 + 0] = (float) (pr.s0 + pr.s1 + pr.s2 + pr.s3) * sc + acc[p * 4 + 0];')"
