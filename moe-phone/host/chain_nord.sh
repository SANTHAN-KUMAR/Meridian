...
log "restart after the doze fix"
...
A "settings put global stay_on_while_plugged_in 7; settings put system screen_off_timeout 1800000; input keyevent KEYCODE_WAKEUP" >/dev/null 2>&1  # svc power stayon did not apply on this Android 12 build
A "cd $H && (BIN=bmoe-v82 CMASK=c0 NT=2 IOMASK=0f SUSTAIN_S=900 setsid nohup sh bmoe_sustain.sh > bmoe_sustain_nohup.log 2>&1 < /dev/null &)" >/dev/null 2>&1
...
A 'settings put global stay_on_while_plugged_in 0; settings put system screen_off_timeout 60000' >/dev/null 2>&1
