...
G2 established two things about OLMoE-1B-7B's routing (see
results/<date>/cache_OLMoE-1B-7B-0924.json):

  - the locality is there:  Belady reaches ~0.50 at a 10% cache, 5x the
    no-locality floor;
  - LRU captures NONE of it at phone-sized caches: 0.000 at 5% and 10%, i.e.
    below the floor, with every expert read missing.

LRU fails here for a structural reason, not a tuning one. Each token touches
top_k experts in each of L layers, so the request stream cycles through far
more distinct experts than a small cache holds before any of them recurs. That
is the cyclic-scan worst case, in which LRU evicts precisely the entry needed
next (the closed form is asserted in tests/test_gates.py::
test_cyclic_scan_lru_zero_belady_closed_form).

This module asks the obvious follow-up: **if evicting is what breaks LRU, what
does a cache that never evicts achieve?** Experts are not used uniformly — per
layer, the most popular quarter of OLMoE's experts take ~41-52% of selections
against 25% under uniform routing — so pinning the popular ones is a real
policy, and it needs no predictor, no classifier and no training.
...
def static_hits(traces, cap_per_layer, warmup_frac=None):
...
def main():
    ...
    seq, T, L, k = cache_sim.request_stream(traces, E)
    ...
    res = {"npz": os.path.abspath(a.npz), "tokens": T, "layers": L, "top_k": k,
           "num_experts": E, "warmup_frac": a.warmup_frac, "rows": []}
    print(f"{'cache':>6} {'floor':>7} {'LRU':>7} {'static-warmup':>14} {'static-oracle':>14} {'Belady':>8}")
    for f in [float(x) for x in a.fractions.split(",")]:
        cap_total = max(1, int(round(f * E * L)))
        cap_layer = per_layer_capacity(cap_total, L)
        lru = cache_sim.lru_hits(seq, cap_total) / seq.size
        bel = cache_sim.belady_hits(seq, cap_total) / seq.size
        s_or, _ = static_hits(traces, cap_layer)
        s_wu, _ = static_hits(traces, cap_layer, warmup_frac=a.warmup_frac)
        print(f"{f:>6.2f} {f:>7.3f} {lru:>7.3f} {s_wu:>14.3f} {s_or:>14.3f} {bel:>8.3f}")
        res["rows"].append({"cache_fraction": f, "cap_total": cap_total,
                            "cap_per_layer": cap_layer, "floor": f, "lru": lru,
                            "static_warmup": s_wu, "static_oracle": s_or, "belady": bel})

    top_n = per_layer_capacity(max(1, int(round(0.10 * E * L))), L)
    ...
    name = "expert_policy_" + os.path.basename(a.npz).replace("traces_", "").replace(".npz", "")
    if a.out_dir:
