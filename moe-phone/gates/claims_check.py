...
CLAIMS = [
    ...
    dict(id="order3_balanced_idorder",
         text="the BALANCED expert-order A/B (ABBA within each repeat, 6 rows per arm): ascending expert id 6.551 tok/s median",
         artifact="bmoe_order3.json", expected=6.551, tol=0.01,
         value=lambda A: next(c["decode_tok_s_median"] for c in A["border3"]["cells"] if c["cell"] == "idorder")),
    dict(id="order3_balanced_residentfirst",
         text="resident-first 6.617 tok/s, i.e. +1.0% against arms whose rows overlap almost entirely and a design that resolves about 6%: no effect, and the +6.0% seen in the unbalanced campaign was its rotation",
         artifact="bmoe_order3.json", expected=6.617, tol=0.01,
         value=lambda A: next(c["decode_tok_s_median"] for c in A["border3"]["cells"] if c["cell"] == "residentfirst")),
    ...
    dict(id="msweep_gpu_expert_gate_up_speedup",
         text="on the GENERIC OpenCL MUL_MAT_ID path the Adreno GPU runs expert gate/up at 1.09x the CPU when resident, with an upload that is a COPY costing 162x its compute. Scope: the bench named its weights weight0..N, and ggml-opencl only uses its Adreno MoE kernels for tensors whose names contain ffn and exps (ggml-opencl.cpp use_adreno_moe_kernels), so the engine's GPU expert path and zero-copy upload were NOT measured; the sweep's CPU clock caps were not logged",
    ...
    dict(id="msweep_gpu_expert_down_incorrect",
         text="the GENERIC OpenCL MUL_MAT_ID fallback (not the Adreno MoE kernels the engine would use, see msweep_gpu_expert_gate_up_speedup) disagrees with the CPU on the expert down-projection in both repeats, max relative error 16.8; the Adreno MoE path's correctness is untested",
    ...
    dict(id="specgate_precision_r2",
         text="the gates are PRECISE -- at rank<2, 406 of the 425 speculated experts actually read in are used (95.5%; 96-99% across all four gates, against 63% ungated) -- but they keep only 9-32% of the ungated arm's useful speculated experts, and the stall follows the useful count, so they fail on stall",
         artifact="specgate_summary.json", expected=0.9553, tol=0.001,
         value=lambda A: A["sgate"]["cells"]["r2"]["spec_useful_median"] / A["sgate"]["cells"]["r2"]["spec_integrated_median"]),
    dict(id="specgate_sel_precision",
    ...
    # --- 2026-09-18: expert quant types per layer, the corrected ledger split, and gx (M3) acceptance
    dict(id="gguf_down_q4_1_layers",
         text="ffn_down_exps is Q4_1 only in layers 0-5 and Q4_0 in layers 6-47 (6 of 48 layers are Q4_1)",
         artifact="gguf_expert_types.json", expected=6, tol=0,
         value=lambda A: len(A["types"]["down_q4_1_layers"])),
    dict(id="ledger_down_q4_0_median_GBps",
         text="on the 42 same-type layers down runs at a median 24.0 GB/s per byte",
         artifact="ceiling_ledger.json", expected=24.0, tol=0.05,
         value=lambda A: A["ledger"]["node_ledger"]["rate_by_layer_type_GB_s"]["down_Q4_0"]["median"]),
    dict(id="ledger_up_same_layers_median_GBps",
         text="vs up at 15.7 GB/s on the same layers",
         artifact="ceiling_ledger.json", expected=15.7, tol=0.05,
         value=lambda A: A["ledger"]["node_ledger"]["rate_by_layer_type_GB_s"]["up_layers_down_Q4_0"]["median"]),
    dict(id="ledger_down_q4_1_median_GBps",
         text="down's rate is the same on its Q4_1 layers (23.5 GB/s)",
         artifact="ceiling_ledger.json", expected=23.5, tol=0.05,
         value=lambda A: A["ledger"]["node_ledger"]["rate_by_layer_type_GB_s"]["down_Q4_1"]["median"]),
    dict(id="ledger_up_vs_down_same_type_ratio",
         text="down still runs 1.53x faster per byte than up at the same Q4_0 type",
         artifact="ceiling_ledger.json", expected=1.53, tol=0.005,
         value=lambda A: A["ledger"]["node_ledger"]["up_vs_down_same_type_Q4_0_median_ratio"]),
    dict(id="ledger_best_case_tok_s",
         text="best case, both levers landing perfectly: about 7.9 tok/s",
         artifact="ceiling_ledger.json", expected=7.9, tol=0.05,
         value=lambda A: A["ledger"]["projection"]["best_case_tok_s_width_plus_gate_up"]),
    dict(id="gx_down_values_compared",
         text="gx down outputs compared bit for bit with ggml-cpu ARM (qemu) over 27 cases",
         artifact="gx_m3_*/gx_test.json", expected=262144, tol=0,
         value=lambda A: A["gx"]["down_values"]),
    dict(id="gx_down_diff_bits",
         text="0 of them differ in any bit",
         artifact="gx_m3_*/gx_test.json", expected=0, tol=0,
         value=lambda A: A["gx"]["down_diff_bits"]),
    dict(id="gx_h_diff_bits",
         text="0 of 98,304 SwiGLU values differ",
         artifact="gx_m3_*/gx_test.json", expected=0, tol=0,
         value=lambda A: A["gx"]["h_diff_bits"]),
    dict(id="gx_quant_mismatch_blocks",
         text="0 of 65,536 crafted near-tie blocks differ from ggml's quantize_row_q8_0/q8_1 (GPU and host paths)",
         artifact="gx_m3_*/gx_test.json", expected=0, tol=0,
         value=lambda A: A["gx"]["gpu_q8_0_mismatch"] + A["gx"]["gpu_q8_1_mismatch"] + A["gx"]["host_q8_0_mismatch"]),
    dict(id="gx_row_down_diff_bits",
         text="the row-per-work-item variant: 0 of 262,144 down values differ from ggml-cpu ARM",
         artifact="gx_m3_*/gx_test.json", expected=0, tol=0,
         value=lambda A: A["gx"]["down_diff_bits_row"] if A["gx"]["down_values_row"] == 262144 else -1),
    dict(id="gx_row_h_diff_bits",
         text="and 0 of 98,304 SwiGLU values differ",
         artifact="gx_m3_*/gx_test.json", expected=0, tol=0,
         value=lambda A: A["gx"]["h_diff_bits_row"] if A["gx"]["h_values_row"] == 98304 else -1),
    dict(id="gx_swiglu_mismatches",
         text="SwiGLU on 1,049,088 crafted gate values over the whole float range (including exp's special branch and its half-integer range-reduction points): 0 differ from ggml ARM's swiglu_split",
         artifact="gx_m3_*/gx_test.json", expected=0, tol=0,
         value=lambda A: A["gx"]["swiglu_mismatches"] if A["gx"]["swiglu_n"] == 1049088 else -1),
    dict(id="thermal_caps_samples",
         text="934 unique thermal-gate samples logged by the phone runs",
         artifact="thermal_caps.json", expected=934, tol=0,
         value=lambda A: A["tcaps"]["samples"]),
    dict(id="thermal_caps_hwmax_below_32c",
         text="the caps sit at the hardware maximum (3.32 / 3.80 GHz) in 78% of samples below 32 C",
         artifact="thermal_caps.json", expected=0.78, tol=0.005,
         value=lambda A: A["tcaps"]["by_front_temperature"]["0-32C"]["fraction_at_hardware_max"]),
    dict(id="thermal_caps_40_42c_policy0",
         text="median policy0 cap at 40-42 C front temperature: 1.90 GHz",
         artifact="thermal_caps.json", expected=1.90, tol=0.005,
         value=lambda A: A["tcaps"]["by_front_temperature"]["40-42C"]["cap0_median_ghz"]),
    dict(id="gx_div_mismatches",
         text="the kernels' correctly rounded division matches IEEE division on 16.8M operand pairs",
         artifact="gx_m3_*/gx_test.json", expected=0, tol=0,
         value=lambda A: A["gx"]["div_mismatches"]),
...
def load_all():
    return {
        ...
        "types": load("gguf_expert_types.json"),
        "ledger": load("ceiling_ledger.json"),
        "gx": load("gx_m3_*/gx_test.json"),
        "tcaps": load("thermal_caps.json"),
