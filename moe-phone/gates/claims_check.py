...
CLAIMS = [
    ...
    dict(id="levers_reference",
         ...
         value=lambda A: next(c["decode_tok_s_median"] for c in A["bmoe"]["cells"] if c["cell"] == "reference")),
    dict(id="levers_cache_big",
         ...
         value=lambda A: next(c["decode_tok_s_median"] for c in A["bmoe"]["cells"] if c["cell"] == "cache_big")),
    dict(id="levers_t6_collapse",
         ...
         value=lambda A: next(c["decode_tok_s_median"] for c in A["bmoe"]["cells"] if c["cell"] == "t6")),
    dict(id="repack_plain",
         ...
         value=lambda A: next(c["decode_tok_s_median"] for c in A["bmoe"]["cells"] if c["cell"] == "i8mm_plain")),
    dict(id="repack_compute",
         ...
         value=lambda A: next(c["compute_s_per_token_median"] for c in A["bmoe"]["cells"] if c["cell"] == "i8mm_repack")),
    dict(id="pin_recycle_best",
         text="pinned 4 compute threads + separate I/O cores + page recycling: 5.04 tok/s median (n=2, fixed cell order, throttled CPU)",
         artifact="bmoe_pin.json", expected=5.036, tol=0.01,
         value=lambda A: next(c["decode_tok_s_median"] for c in A["bmoe"]["cells"] if c["cell"] == "pin_t4_recycle")),
    dict(id="pin_unpinned_median",
         text="the unpinned baseline in the same campaign: 3.13 tok/s median, one repeat collapsed to 1.58",
         artifact="bmoe_pin.json", expected=3.1325, tol=0.01,
         value=lambda A: next(c["decode_tok_s_median"] for c in A["bmoe"]["cells"] if c["cell"] == "unpinned_t4_io4")),
